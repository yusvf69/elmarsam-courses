...
const handleEnroll = async (courseId: string) => {
  ...
  // Show enroll buttons only when we're sure user is not enrolled
  return (
    <div className="flex gap-2 w-full">
      <button
        className={`flex-1 rounded-lg px-6 py-3 font-medium transition-all duration-300 ease-in-out relative h-12
          ${
            isPending || !user?.id
